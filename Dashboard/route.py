...
def register_page():
    ...
    if form.validate_on_submit():
        user_to_create =User(phone=form.phone.data,username=form.username.data,email_address=form.email.data,password=form.password1.data)
...
def login_page():
    ...
    #     attempted_password= User.query.filter_by(password=form.password.data).first()
        attempted_user = User.query.filter_by(phone=form.phone.data).first()
